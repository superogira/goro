...
func (r *Renderer) NewTextureFromRGBAWithOptions(width, height int, data []byte, opts TextureOptions) (*Texture, error) {
	...
	// Upload pixel data via wgpu Queue. WebGPU requires bytesPerRow to be a
	// multiple of 256 for writeTexture (except single-row copies), so rows
	// are repacked with padding when width*4 is not aligned. Native backends
	// accept the raw layout, but the padded path is valid everywhere.
	rowBytes := width * 4
	bytesPerRow := rowBytes
	upload := data
	if height > 1 && rowBytes%256 != 0 {
		bytesPerRow = (rowBytes + 255) / 256 * 256
		padded := make([]byte, bytesPerRow*height)
		for row := 0; row < height; row++ {
			copy(padded[row*bytesPerRow:row*bytesPerRow+rowBytes], data[row*rowBytes:(row+1)*rowBytes])
		}
		upload = padded
	}
