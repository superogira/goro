...
func (m *WorldMode) preloadMapTextures(ctx client.Context) {
	...
	if gnd != nil {
		...
		// This branch's GND cache carries the handheld's world-aligned
		// ground lightmap (soft shadows) instead of upstream's tile atlas.
		lightmap := buildGNDGroundLightmap(gnd)
		add(m.ownMapImage(lightmap.image))
		m.gndMeshCache = &gndRetainedMeshCache{gnd: gnd, rsw: rsw, groundLightmap: lightmap, chunks: make(map[gndRetainedChunkKey][]retainedWorldMesh)}
	}
	if rsw != nil {
