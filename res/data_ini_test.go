...
func TestManagerDefaultArchives(t *testing.T) {
	for _, tc := range []struct {
		...
		// This branch's last-tier fallback: with no DATA.INI and no
		// standard archive at all, custom root-level archives load
		// (sorted by priority) — the handheld's data folder is exactly
		// this shape. A DATA.INI, even an empty [Data], stays explicit.
		{"no standard archives", nil, []string{"00-custom.grf", "event.grf", "patch.gpf"}},
	} {
