...
func TestBasicInfoShortcut(t *testing.T) {
	...
	before := m.ui.characterWindow.Compact()
	ctx.Input.SetKeyCode(gpucontext.KeyLeftAlt, true)
	ctx.Input.SetKeyCode(gpucontext.KeyV, true)
	if !m.PrepareTextInput(ctx, gpucontext.KeyV) || !m.toggleWindowFromInput(ctx) || m.ui.characterWindow.Compact() == before {
		t.Fatal("Alt+V did not change the basic-info layout")
	}
}
