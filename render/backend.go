...
func (r *runner) draw(ctx *gogpu.Context) error {
	...
	// Goro redraws the 3D scene every frame; the "goro" damage source
	// reports full-surface damage after submit (ADR-065), so UI canvas
	// damage only scopes UI texture updates.
...
func (r *runner) updateFPSCounter(now time.Time) {
	...
	if webFPSReady() {
		// The DOM meter is the web's only overlay: mirror the memory line
		// into it as a second line instead of the canvas text box.
		webText := r.fpsText
		if r.memText != "" {
			webText += "\n" + r.memText
		}
