...
func (m *Manager) scanKnownFiles() error {
	...
	// Last-tier fallback, this branch only: the handheld's data folder
	// ships no DATA.INI and carries custom-named archives (data-full.grf
	// & co). When neither DATA.INI nor any standard archive was found,
	// load every root-level .grf/.gpf — without this the device boots
	// with an empty resource set. A present-but-selecting DATA.INI (even
	// an empty [Data]) is always respected as-is.
	if hadDataINI || len(paths) > 0 {
		return nil
	}
	if entries, err := os.ReadDir(m.Root); err == nil {
		var extras []string
		...
		sort.SliceStable(extras, func(i, j int) bool {
			return archivePriority(extras[i]) < archivePriority(extras[j])
		})
