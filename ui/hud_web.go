...
func hudWebInstallHooks() {
	...
	js.Global().Set("goroSkillAction", js.FuncOf(func(this js.Value, args []js.Value) any {
		if len(args) >= 1 && args[0].Type() == js.TypeString {
			hudWebActionQueue.Lock()
			hudWebActionQueue.actions = append(hudWebActionQueue.actions, "skill:"+args[0].String())
			hudWebActionQueue.Unlock()
		}
		return nil
	}))
	...
	js.Global().Set("goroLoginAction", js.FuncOf(func(this js.Value, args []js.Value) any {
		if len(args) >= 1 && args[0].Type() == js.TypeString {
			hudWebActionQueue.Lock()
			hudWebActionQueue.actions = append(hudWebActionQueue.actions, "login:"+args[0].String())
			hudWebActionQueue.Unlock()
		}
		return nil
	}))
	js.Global().Set("goroServiceAction", js.FuncOf(func(this js.Value, args []js.Value) any {
		if len(args) >= 1 && args[0].Type() == js.TypeString {
			hudWebActionQueue.Lock()
			hudWebActionQueue.actions = append(hudWebActionQueue.actions, "svc:"+args[0].String())
			hudWebActionQueue.Unlock()
		}
		return nil
	}))
	...
	js.Global().Set("goroCharSelectAction", js.FuncOf(func(this js.Value, args []js.Value) any {
		if len(args) >= 1 && args[0].Type() == js.TypeString {
			hudWebActionQueue.Lock()
			hudWebActionQueue.actions = append(hudWebActionQueue.actions, "char:"+args[0].String())
			hudWebActionQueue.Unlock()
		}
		return nil
	}))
...
// the title screen runs before the in-world windows that normally install
// them lazily.
