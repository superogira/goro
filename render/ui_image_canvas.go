...
// WindowSnapshotter is the optional canvas capability positionedOverlay uses
// to cache whole UI windows as pixels: capture once after painting, blit on
// every later repaint that has no changes (window reopen after close).
type WindowSnapshotter interface {
	// SnapshotRect captures the pixels covering the logical rect r.
	SnapshotRect(r geometry.Rect) image.Image
	// BlitSnapshot draws a previously captured physical-resolution image
	// so it exactly covers the logical rect r (no resampling).
	BlitSnapshot(img image.Image, r geometry.Rect)
}

type physicalRectCopier interface {
	CopyRect(image.Rectangle) *image.RGBA
}

func (c scaledImageCanvas) SnapshotRect(r geometry.Rect) image.Image {
	copier, ok := c.Canvas.(physicalRectCopier)
	if !ok || c.Canvas == nil || r.IsEmpty() {
		return nil
	}
	sc := float64(c.scale)
	phys := image.Rectangle{
		Min: image.Point{X: int(math.Round(float64(r.Min.X) * sc)), Y: int(math.Round(float64(r.Min.Y) * sc))},
		Max: image.Point{X: int(math.Round(float64(r.Max.X) * sc)), Y: int(math.Round(float64(r.Max.Y) * sc))},
	}
	return copier.CopyRect(phys)
}

func (c scaledImageCanvas) BlitSnapshot(img image.Image, r geometry.Rect) {
	if img == nil || c.Canvas == nil || r.IsEmpty() {
		return
	}
	rgba, ok := img.(*image.RGBA)
	if !ok {
		c.Canvas.DrawImage(img, geometry.Pt(r.Min.X, r.Min.Y))
		return
	}
	...
	scImg := &scene.Image{Width: w, Height: h, Data: rgba.Pix}
	s := scene.NewScene()
	s.DrawImage(scImg, scene.NewAffine(r.Width()/float32(w), 0, r.Min.X, 0, r.Height()/float32(h), r.Min.Y))
	c.Canvas.ReplayScene(s)
}
